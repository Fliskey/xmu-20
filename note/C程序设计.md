...
找代码的时候，沿着问题表现形式到问题的根源来分析代码，不要沿着不相关路径

要充分利用调试器和编译器给出的警告或输出

对于大型且组织良好的系统，只需最低限度了解全部功能，就能对它做出修改

增加功能时，先找到实现类似特性的代码，作为模板

从特性的功能描述到代码的实现可以按照字符串消息，或使用关键词来搜索代码

在一直代码或秀爱接口时，可以通过编译器直接定位问题涉及的范围，从而减少代码阅读的工作量

重构时，从一个能够正常工作的系统开始做起，希望确保结束时系统能够正常工作。善用测试用例

代码重构时，先从系统的构架开始，然后逐步细化

复查软件系统时，注意系统时由多部分组成的，不仅时执行语句，还要注意分析文件和结构目录、生成和配置过程、用户界面和系统的文档
...
- 做一个影评记录程序，输入用户看过的电影列表，记录每部的片名、评价等
